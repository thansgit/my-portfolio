...
export function AboutSection() {
  const services: ServiceItem[] = [
    {
      ...
      icon: <FaLaptopCode className='h-8 w-8 text-yellow-500' />,
    },
    {
      title: 'Backend Development',
      description: 'Working on server-side logic and APIs.',
      icon: <FaServer className='h-8 w-8 text-yellow-500' />,
    },
    {
      title: 'UI/UX Design',
      description: 'Designing functional and intuitive interfaces.',
      icon: <FaPaintBrush className='h-8 w-8 text-yellow-500' />,
    },
    {
  ...
  const skills = [
    { name: 'React', icon: <FaReact className='h-8 w-8 text-yellow-500' /> },
    { name: 'Node.js', icon: <FaNodeJs className='h-8 w-8 text-yellow-500' /> },
    { name: 'TypeScript', icon: <SiTypescript className='h-8 w-8 text-yellow-500' /> },
    { name: 'Django', icon: <SiDjango className='h-8 w-8 text-yellow-500' /> },
    { name: 'Kotlin', icon: <SiKotlin className='h-8 w-8 text-yellow-500' /> },
    { name: 'Python', icon: <FaPython className='h-8 w-8 text-yellow-500' /> },
    { name: 'Three.js', icon: <SiThreedotjs className='h-8 w-8 text-yellow-500' /> },
    { name: 'Cypress', icon: <SiCypress className='h-8 w-8 text-yellow-500' /> },
  ]
  ...
  return (
    <Section title='About Me'>
      ...
      <div className='mb-12 space-y-16'>
        {/* Bio Card */}
        <div
          className={`/* [Bio Card] Enhanced card with gradient background */ rounded-xl border border-zinc-700 bg-gradient-to-b from-zinc-800 to-zinc-900 p-6 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1),0_8px_16px_rgba(0,0,0,0.4)] backdrop-blur-sm`}
        >
          <div className='space-y-5 text-zinc-300'>
            <div className='flex items-start gap-4'>
              {/* Bio Icon with Gradient */}
              <div
                className={`/* [Icon Container] Embossed icon with gradient */ mt-1 flex-shrink-0 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600 p-2 shadow-[0_2px_4px_rgba(0,0,0,0.3),inset_0_1px_2px_rgba(255,255,255,0.4)]`}
              >
                <svg
        ...
        <div>
          ...
          <div className='grid grid-cols-2 gap-6 sm:grid-cols-4'>
            {skills.map((skill) => (
              <div
                key={skill.name}
                className={`/* [Skill Card] Individual skill display */ flex flex-col items-center rounded-lg border border-zinc-700 bg-zinc-800/50 p-4`}
              >
                {skill.icon}
